...
ice::ast::type::type(const string_list& name, const type_list& specializations, int pointer)
    : _name(name), _specializations(specializations), _pointer(pointer)
{
    _display_name = ice::utils::join(".", _name);
}
...
const char *
ice::ast::type::get_display_name() const
{
